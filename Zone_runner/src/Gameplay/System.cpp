...
void CollisionSystem::update(std::vector<Entity *>const &entity1, std::vector<Entity *>const &entity2) {
    for (auto it = entity1.begin(); it != entity1.end(); it++) {
        auto ent1 = *it;
        for (auto it2 = entity2.begin(); it2 != entity2.end(); it2++) {
            auto ent2 = *it2;
            if (CheckCollisionRecs(ent1->getComponent<CollisionComponent>()->hitbox, ent2->getComponent<CollisionComponent>()->hitbox)) {
                ent1->getComponent<CollisionComponent>()->isTouch = true;
                ent2->getComponent<CollisionComponent>()->isTouch = true;
                return;
            } else {
                ent1->getComponent<CollisionComponent>()->isTouch = false;
                ent2->getComponent<CollisionComponent>()->isTouch = false;
            }
        }
    }
}

void JumpSystem::update(std::vector<Entity *>const & entities, PlateformSystem &Plateform) {
     for (auto& e : entities) {
        auto position = e->getComponent<PositionComponent>();
        ...
        std::cout << "ground =" << jump->_ground << std::endl;
        std::cout << "max = " << jump->_ground - jump->_max << std::endl;
        if (_state == 0) {
            if (IsKeyDown(SPACE)) {
                _state = 1;
            ...
            if (position->y - jump->_speed  <= jump->_ground - jump->_max) {
                _state = 2;
                std::cout << "up up and away" << std::endl;
            } else {
                for (auto& p : Plateform.MyPlateforms) {
                    collision.update(entities, p._plateform);
                    if (col->isTouch == true) {
                        _state = 2;
                        position->y = col->hitbox.y - 5;
                        std::cout << "plateform hit from up" << std::endl;
                    } else 
                        position->y -= jump->_speed;
                }
            }
        } else if (_state == 2) {
            if (position->y + jump->_fall >= jump->_ground) {
                _state = 0;
                position->y = jump->_ground;
                std::cout << "the bigger they are the harder they fall" << std::endl;
            } else {
                for (auto& p : Plateform.MyPlateforms) {
                    collision.update(entities, p._plateform); 
                    if (col->isTouch == true) {
                        _state = 0;
                        jump->_where = position->y;
                        std::cout << "plateform hit from down" << std::endl;
                    } else 
                        position->y += jump->_speed;
                }
            }
